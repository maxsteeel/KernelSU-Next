			 ...
			 struct perm_datum *perm, int effect, bool invert)
{
    if (src == NULL) {
        ...
        if (strip_av(effect, invert)) {
		    ...
		    if (type->attribute) {
			    add_rule_raw(db, type, tgt, cls, perm, effect,
					 invert);
		    }
	    };
        }
        ...
        if (strip_av(effect, invert)) {
		    ...
		    struct type_datum *type =
			    (struct type_datum *)(node->datum);
		    if (type->attribute) {
			    add_rule_raw(db, src, type, cls, perm, effect,
					 invert);
		    }
	    };
        }
        ...
        {
		add_rule_raw(db, src, tgt, (struct class_datum *)node->datum,
			     perm, effect, invert);
	}
    } else {
        struct avtab_key key;
			       ...
			       bool invert)
{
    if (src == NULL) {
		...
		if (type->attribute) {
			add_xperm_rule_raw(db, type, tgt, cls, low, high,
					   effect, invert);
		}
	};
    } else if (tgt == NULL) {
		...
		struct type_datum *type = (struct type_datum *)(node->datum);
		if (type->attribute) {
			add_xperm_rule_raw(db, src, type, cls, low, high,
					   effect, invert);
		}
	};
    } else if (cls == NULL) {
		...
		add_xperm_rule_raw(db, src, tgt,
				   (struct class_datum *)(node->datum), low,
				   high, effect, invert);
	};
    } else {
        struct avtab_key key;
			       ...
			       const char *o)
{
    ...
    if (trans == NULL) {
	    ...
	    struct filename_trans *new_key = (struct filename_trans *)kzalloc(
		    sizeof(*new_key), GFP_ATOMIC);
	    if (!new_key) {
